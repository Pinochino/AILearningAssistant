...
export function ContentManagement() {
  ...
  const fetchAllMaterials = async () => {
    try {
      ...
      const materialsData =
        response.data?.data?.items || response.data?.items || response.data?.data || response.data || []
  ...
  const ClassUserInfo: React.FC<{ classId: string; userId?: any; uploadedBy?: any }> = ({
    classId,
    userId,
    uploadedBy
  }) => {
  ...
  const filteredDocuments =
    selectedClass === 'all' ? documents : documents.filter((doc) => doc.classId === selectedClass)

  const filteredQuizzes = selectedClass === 'all' ? quizzes : quizzes.filter((quiz) => quiz.classId === selectedClass)

  const filteredFlashcards =
    selectedClass === 'all' ? flashcards : flashcards.filter((flashcard) => flashcard.classId === selectedClass)
                  ...
                  <div className='text-center py-8 text-muted-foreground'>
                    ...
                    <p>
                      {selectedClass === 'all'
                        ...
                        : 'Không có quiz nào cho lớp học này'}
                    </p>
                  </div>
                ) : (
              ...
              <div className='space-y-4'>
                {loading ? (
                  ...
                  <div className='text-center py-8 text-muted-foreground'>
                    ...
                    <p>
                      {selectedClass === 'all'
                        ? 'Chưa có flashcard nào trong hệ thống'
                        : 'Không có flashcard nào cho lớp học này'}
                    </p>
                  </div>
                ) : (
