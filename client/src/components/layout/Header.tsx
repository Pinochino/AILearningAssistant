...
export function Header({ onToggleSidebar }: HeaderProps) {
  ...
  const { user, loading, error } = useAppSelector((state: RootState) => state.auth.login)
  ...
  const getRoleLabel = (role: string) => {
    ...
    if (Array.from(roles).includes('STUDENT')) {
      return 'Học sinh'
    }
    return null
  }
    ...
    <header className='bg-background border-b border-border px-4 py-3'>
      <div className='flex items-center justify-between gap-4'>
        <div className='flex items-center gap-4 flex-1'>
          <Button variant='ghost' size='sm' onClick={onToggleSidebar} className='flex-shrink-0'>
            <Menu className='h-5 w-5' />
          </Button>

          <div className='hidden md:flex relative flex-1 max-w-3xl'>
            <Search className='absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-muted-foreground' />
            <Input placeholder='Tìm kiếm môn học, tài liệu, quiz...' className='pl-9 bg-muted/30 w-full' />
          </div>
        </div>
