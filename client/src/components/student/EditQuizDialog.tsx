...
export function EditQuizDialog({ isOpen, onOpenChange, quiz, onSave }: EditQuizDialogProps) {
  ...
  const [questions, setQuestions] = useState<QuizQuestion[]>(quiz?.questions?.length ? quiz.questions : mockQuestions)
  const [visibility, setVisibility] = useState<'private' | 'public'>(quiz?.isPublic ? 'public' : 'private')
      ...
      <DialogContent className='max-w-5xl max-h-[90vh] overflow-y-auto'>
        ...
        <div className='space-y-6'>
          <div className='space-y-2'>
            ...
            <Input placeholder='Nhập tiêu đề quiz' value={title} onChange={(e) => setTitle(e.target.value)} />
          </div>
