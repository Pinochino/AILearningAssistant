...
const PlayQuiz = () => {
      ...
      <div className='min-h-screen bg-gradient-to-br from-red-50 via-pink-50 to-rose-50 flex items-center justify-center'>
        <Card className='w-full max-w-md'>
          <CardContent className='p-6 text-center'>
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className='text-red-500 text-6xl mb-4'>
              ⚠️
            </motion.div>
  ...
  if (!quiz) {
    return (
      <div className='min-h-screen bg-gradient-to-br from-gray-50 via-slate-50 to-zinc-50 flex items-center justify-center'>
        <Card className='w-full max-w-md'>
          <CardContent className='p-6 text-center'>
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className='text-gray-400 text-6xl mb-4'>
              📝
            </motion.div>
  ...
  return (
    <div
      style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 25%, #f093fb 50%, #f5576c 75%, #fda085 100%)' }}
      className={`${isFullscreen ? 'fixed inset-0 z-50' : 'min-h-screen'} p-4`}
    >
        ...
        >
          <div className='flex items-center justify-between mb-4 relative'>
            ...
            <div className='flex items-center gap-4 mt-2'>
              ...
              <div
                style={{ color: 'white' }}
                className={`flex items-center ${timer >= (quiz?.duration || 30) * 60 - 60 ? 'text-yellow-300 font-bold' : ''}`}
              >
                        ...
                        return (
                          ...
                          >
                            ...
                            {!showResult && !isSelected && !isDisabled && (
                              <div
                                className={`absolute inset-0 bg-gradient-to-r ${colorClass} opacity-0 hover:opacity-10 transition-opacity`}
                              />
                            )}
