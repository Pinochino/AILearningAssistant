...
export function StudentQuizFlashcard() {
  ...
  const handleSaveQuiz = (updatedQuiz: any) => {
    setPrivateQuizzes(privateQuizzes.map((quiz) => (quiz.id === updatedQuiz.id ? updatedQuiz : quiz)))
    console.log('Quiz updated:', updatedQuiz)
  }

  const handleSaveFlashcard = (updatedFlashcard: any) => {
    setPrivateFlashcards(
      privateFlashcards.map((flashcard) => (flashcard.id === updatedFlashcard.id ? updatedFlashcard : flashcard))
    )
    ...
    <div className='space-y-6'>
      ...
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-muted-foreground'>Tạo và chia sẻ quiz, flashcard với bạn bè</p>
        </div>
                    ...
                    <div className='space-y-4'>
                      ...
                      <RadioGroup value={visibility} onValueChange={setVisibility} className='flex flex-row gap-6'>
                        <div className='flex items-center space-x-2 cursor-pointer'>
                          <RadioGroupItem value='private' id='private' className='w-4 h-4 border border-gray-400' />
                          <Label htmlFor='private' className='font-normal text-gray-700 cursor-pointer'>
                            Chỉ mình tôi
                          </Label>
                        </div>
                        <div className='flex items-center space-x-2 cursor-pointer'>
                          <RadioGroupItem value='public' id='public' className='w-4 h-4 border border-gray-400' />
                          <Label htmlFor='public' className='font-normal text-gray-700 cursor-pointer'>
                            Công khai
                          </Label>
                        </div>
                      </RadioGroup>
                  ...
                  <Card key={flashcard.id}>
                    <CardContent className='p-4'>
                      <div className='flex items-center justify-between'>
                        ...
                        <div className='flex items-center gap-2'>
                          <Button variant='outline' size='sm' onClick={() => handleEditFlashcard(flashcard)}>
                            <Edit className='h-4 w-4' />
                          </Button>
                          <Button variant='outline' size='sm'>
                            <Share2 className='h-4 w-4' />
                          </Button>
                          <Button variant='outline' size='sm' onClick={() => handleDeleteFlashcard(flashcard.id)}>
                            <Trash2 className='h-4 w-4' />
                          </Button>
                          <Button size='sm'>
                            <Play className='h-4 w-4 mr-2' />
