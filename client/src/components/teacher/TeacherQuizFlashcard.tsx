          ...
          <CardContent className='p-4'>
            <div className='flex items-center gap-3'>
              ...
              <div>
                ...
                <p className='text-xl font-semibold'>{mockQuizzes.reduce((sum, quiz) => sum + quiz.attempts, 0)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <TabsContent value='activity' className='space-y-4'>
          <Card>
            <CardHeader>
              ...
              <CardDescription>Theo dõi hoạt động của học sinh với quiz và flashcard</CardDescription>
            </CardHeader>
            <CardContent>
              <div className='space-y-4'>
                {mockRecentActivity.map((activity) => (
                  <div key={activity.id} className='flex items-center gap-3 p-3 bg-muted/30 rounded-lg'>
                    ...
                    {activity.type === 'quiz_attempt' && (
                      <Badge variant={activity.score >= 80 ? 'default' : 'secondary'}>{activity.score} điểm</Badge>
                    )}
      ...
      <Dialog open={shareDialogOpen} onOpenChange={setShareDialogOpen}>
        <DialogContent>
          ...
          <div className='space-y-4'>
            <Alert>
              <AlertDescription>
                Link chia sẻ sẽ cho phép người khác xem và sử dụng {selectedTab === 'quizzes' ? 'quiz' : 'flashcard'}{' '}
                này.
              </AlertDescription>
            </Alert>
            ...
            <div className='space-y-2'>
              ...
              <div className='flex gap-2'>
                <Input value={`${window.location.origin}/share/${selectedItem?.id}`} readOnly className='flex-1' />
