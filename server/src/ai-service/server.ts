...
async function callGeminiRobust(prompt: string): Promise<string> {
  ...
  const attempts = [
    ...
    {
      url: `https://generativelanguage.googleapis.com/v1/models/gemini-flash-latest:generateContent?key=${GEMINI_API_KEY}`,
      version: 'v1',
      model: 'gemini-flash-latest'
    },
    {
      url: `https://generativelanguage.googleapis.com/v1/models/gemini-pro-latest:generateContent?key=${GEMINI_API_KEY}`,
      version: 'v1',
  ...
  for (const att of attempts) {
    try {
      ...
      console.warn(
        `Attempt failed (${att.version}/${att.model})`,
